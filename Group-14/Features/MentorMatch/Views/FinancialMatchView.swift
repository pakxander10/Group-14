...
struct FinancialMatchView: View {
    ...
    private var stepContent: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                case 2: firstGenStep
                case 3: financialStandingStep
                case 4: confidenceStep
                case 5: financialSpecificsStep
                case 6: mentorPreferencesStep
                default: EmptyView()
                }
            }
            .padding(.horizontal, 20)
    ...
    private let intentOptions: [MatchOption] = [
        .init(label: "I just got my first job offer and I don't know what to do", value: "first_job", emoji: "📩"),
        .init(label: "I feel behind financially and I don't know where to start", value: "feel_behind", emoji: "😟"),
        .init(label: "I'm about to graduate and I'm worried about money", value: "grad_worried", emoji: "🎓"),
        .init(label: "I want to start investing but have no idea how", value: "investing", emoji: "📈"),
    ...
    private var whoYouAreStep: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                TextField("e.g. Business, Nursing, Undecided…", text: $vm.selectedMajor)
                    .padding(14)
    ...
    private let genderOptions: [MatchOption] = [
        .init(label: "Male", value: "man", emoji: "👤"),
        .init(label: "Female", value: "woman", emoji: "👤"),
    ...
    // MARK: - Step 3: Financial Standing

    private var financialStandingStep: some View {
        VStack(alignment: .leading, spacing: 20) {
            stepHeader(title: "What does your money situation look like right now?", subtitle: "Be honest — this is just for matching.")
            ForEach(financialStandingOptions) { option in
                optionCard(option: option, isSelected: vm.selectedFinancialStanding == option.value) {
                    vm.selectedFinancialStanding = option.value
                }
            }
        }
    }

    private let financialStandingOptions: [MatchOption] = [
        .init(label: "I'm a student with very limited income", value: "student_limited", emoji: "🎒"),
        .init(label: "I'm working part-time while in school", value: "part_time", emoji: "⏰"),
        .init(label: "I just started my first full-time job", value: "first_job", emoji: "💼"),
        .init(label: "I'm managing student loans and figuring out next steps", value: "student_loans", emoji: "📋"),
        .init(label: "I have some savings but I'm not sure what to do with them", value: "has_savings", emoji: "💰"),
    ]
    ...
    private var confidenceStep: some View {
        VStack(alignment: .leading, spacing: 20) {
            stepHeader(title: "When it comes to money, how do you feel right now?", subtitle: "Pick the one that feels most true.")
    ...
    // MARK: - Step 5: Financial Specifics

    private var financialSpecificsStep: some View {
        VStack(alignment: .leading, spacing: 24) {
            VStack(alignment: .leading, spacing: 20) {
                stepHeader(title: "Which of these do you currently have?", subtitle: "Select all that apply.")
                ForEach(financialAccountOptions) { option in
                    multiSelectCard(
                        option: option,
                        isSelected: vm.selectedFinancialAccounts.contains(option.value)
                    ) {
                        vm.toggleFinancialAccount(option.value)
                    }
                }
            }
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                ForEach(financialConcernOptions) { option in
                    optionCard(option: option, isSelected: vm.selectedFinancialConcern == option.value) {
                        vm.selectedFinancialConcern = option.value
                    }
                }
            }
        }
    ...
    private let financialAccountOptions: [MatchOption] = [
        ...
        .init(label: "401k or retirement account through work", value: "401k", emoji: "📊"),
        .init(label: "Roth IRA or investment account", value: "roth_ira", emoji: "📈"),
        .init(label: "None of these yet", value: "none", emoji: "🔲"),
    ]

    private let financialConcernOptions: [MatchOption] = [
        .init(label: "Making rent or covering basic expenses", value: "rent", emoji: "🏠"),
        .init(label: "Understanding my first paycheck and taxes", value: "paycheck", emoji: "🧾"),
        .init(label: "Paying off student debt", value: "student_debt", emoji: "📋"),
        .init(label: "Starting to invest but not knowing how", value: "investing", emoji: "📈"),
        .init(label: "Negotiating my salary", value: "salary", emoji: "💬"),
        .init(label: "Just understanding how money actually works", value: "basics", emoji: "🧭"),
    ]

    // MARK: - Step 6: Mentor Preferences
